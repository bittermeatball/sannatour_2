        ...
        <form action="'.$nd_travel_action.'" method="get">
        	...
        	<div style="padding:'.$nd_travel_fields_padding.';" id="nd_travel_search_components_keyword" class=" '.$nd_travel_width.' nd_travel_float_left nd_travel_width_100_percentage_responsive nd_travel_box_sizing_border_box">';
        		...
        		if ( $nd_travel_labels == 'yes' ){
	        		$nd_travel_str .= '
	        			<h6 class="nd_travel_letter_spacing_1">'.__('Keyword','nd-travel').'</h6>
	        			<div class="nd_travel_section nd_travel_height_20"></div>
	        		';
	        	}

	        	$nd_travel_str .= '
        		<input id="nd_travel_vc_search_l1_keyword" name="nd_travel_archive_form_keyword" placeholder="'.__('Bạn muốn tìm gì ?','nd-travel').'" value="" class=" '.$nd_travel_field_class.' nd_travel_section" type="text">
        	</div>';
			...
			<div style="padding:'.$nd_travel_fields_padding.';" id="nd_travel_search_components_destinations" class="'.$nd_travel_width.' nd_travel_float_left nd_travel_width_100_percentage_responsive nd_travel_box_sizing_border_box">';
				...
				if ( $nd_travel_labels == 'yes' ){
	        		$nd_travel_str .= '
	        			<h6 class="nd_travel_letter_spacing_1">'.__('Destination','nd-travel').'</h6>
	        			<div class="nd_travel_section nd_travel_height_20"></div>
	        		';
	        	}
				...
				while ( $nd_travel_destinations_query->have_posts() ) : $nd_travel_destinations_query->the_post();
					...
					if ( $nd_travel_destinations_query_i == 0 ) { $nd_travel_str .= '<option value="0">'.__('Tất cả điểm đến','nd-travel').'</option>'; }
			...
			<div style="padding:'.$nd_travel_fields_padding.';" id="nd_travel_search_components_typlogies" class=" '.$nd_travel_width.' nd_travel_float_left nd_travel_width_100_percentage_responsive nd_travel_box_sizing_border_box">';
				...
				if ( $nd_travel_labels == 'yes' ){
	        		$nd_travel_str .= '
	        			<h6 class="nd_travel_letter_spacing_1">'.__('Typology','nd-travel').'</h6>
	        			<div class="nd_travel_section nd_travel_height_20"></div>
	        		';
	        	}
				...
				while ( $nd_travel_typlogies_query->have_posts() ) : $nd_travel_typlogies_query->the_post();
					...
					if ( $nd_travel_typlogies_query_i == 0 ) { $nd_travel_str .= '<option value="">'.__('Tất cả loại tour','nd-travel').'</option>'; }
		    ...
		    foreach($nd_travel_taxonomies as $nd_travel_tax){
		      ...
		      $nd_travel_str .= nd_travel_build_select($nd_travel_tax,$nd_travel_i,$nd_travel_width,$nd_travel_fields_padding,$nd_travel_field_class,$nd_travel_labels);
    		...
    		<div style="padding:'.$nd_travel_fields_padding.';" id="nd_travel_search_components_submit" class=" '.$nd_travel_submit_width.' nd_travel_float_left nd_travel_width_100_percentage_responsive nd_travel_box_sizing_border_box">';
    			...
    			if ( $nd_travel_labels == 'yes' ){
				    $nd_travel_str .= '
				      <h6 class="nd_travel_letter_spacing_1 nd_travel_opacity_0">'.__('Tìm kiếm','nd-travel').'</h6>
				      <div class="nd_travel_section nd_travel_height_20"></div>
				    ';
				  }

	        	$nd_travel_str .= '
            	<input style="padding: '.$nd_travel_submit_padding.'; background-color:'.$nd_travel_submit_bg.';" class="nd_options_color_white nd_options_second_font_important nd_travel_width_100_percentage " type="submit" value="'.__('TÌM KIẾM','nd-travel').'">
            </div>

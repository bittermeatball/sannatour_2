...
function nd_travel_shortcode_search_results($nd_travel_ssr_atts) {
    ...
    //default value
    if ( $nd_travel_shortcode_search_results['layout'] == '' ) { $nd_travel_shortcode_search_results['layout'] = 'layout-1'; }
    if ( $nd_travel_shortcode_search_results['slider_price_color'] == '' ) { $nd_travel_shortcode_search_results['slider_price_color'] = '#000'; }
    if ( $nd_travel_shortcode_search_results['results_margin'] == '' ) { $nd_travel_shortcode_search_results['results_margin'] = '100'; }
    if ( $nd_travel_shortcode_search_results['filter_bg_color'] == '' ) { $nd_travel_shortcode_search_results['filter_bg_color'] = '#000'; }
    ...
    if ( $nd_travel_shortcode_search_results['price_default_value'] == '' ) { $nd_travel_shortcode_search_results['price_default_value'] = 100000000; }
    if ( $nd_travel_shortcode_search_results['price_max_value'] == '' ) { $nd_travel_shortcode_search_results['price_max_value'] = 100000000; }
    ...
    <input type="hidden" value="'.$nd_travel_shortcode_search_results['layout'].'" name="nd_travel_layout" id="nd_travel_layout">
    ';
    ...
    include 'include/search-results/'.$nd_travel_shortcode_search_results['layout'].'/index.php';
...
function nd_travel_sorting_php() {
    ...
    $nd_travel_layout = $_GET['nd_travel_layout'];
    ...
    <div id="nd_travel_content_result" class="nd_travel_section">';
        ...
        <div id="nd_travel_search_cpt_1_no_results" class="nd_travel_section nd_travel_padding_15 nd_travel_box_sizing_border_box">
            <div class="nd_travel_section nd_travel_bg_yellow nd_travel_padding_15_20 nd_travel_box_sizing_border_box">
              ...
              <h3 class="nd_travel_float_left nd_options_color_white nd_travel_color_white nd_options_first_font nd_travel_margin_left_10">'.__('Không tìm thấy tour mà bạn đã chọn','nd-travel').'</h3>
            </div>
        </div>
        ...
        while ( $the_query->have_posts() ) : $the_query->the_post();
            ...
            include 'include/search-results/'.$nd_travel_layout.'/nd_travel_post_preview-'.$nd_travel_search_filter_layout.'.php';
        ...
        $nd_travel_shortcode_right_content .= '</div>
            ...
            include 'include/search-results/'.$nd_travel_layout.'/nd_travel_search_results_pagination.php';
